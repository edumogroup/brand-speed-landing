...
async function callSheet(action, data) {
  ...
  try {
    ...
    if (!resp.ok || !json || json.error) {
      console.error(`Google Sheet "${action}" error:`, resp.status, json || rawText.slice(0, 300));
      return null;
    }
    return json;
  } catch (err) {
...
const saveSubscriber = (email) => callSheet('save_subscriber', { email });

module.exports = { saveLead, updateReminderId, markPaidByAmount, saveSubscriber };
